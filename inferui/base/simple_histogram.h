...
public:
  ...
  friend std::ostream& operator<<(std::ostream& os, const SimpleHistogram& histogram);
...
inline std::ostream& operator<<(std::ostream& os, const SimpleHistogram& histogram) {
  os << "\ntotal: " << histogram.total_count_ << "\n";
  int count = histogram.counts_.size();
  for (int i = 0; i < count; ++i) {
    double ratio = (histogram.total_count_ > 0) ? (static_cast<double>(histogram.counts_[i]) / histogram.total_count_) : 0.0;
    os << StringPrintf("  %3d%c : %d (%3.1f%%)\n", i, (i==count-1)?'+':' ', histogram.counts_[i], ratio*100);
  }
  os << '\n';
  return os;
}
